...
const { TabPane } = Tabs; // Получаем доступ к TabPane

const CardR = () => {
  const currentUrl = useLocation().pathname;
  const [activeTabKey1, setActiveTabKey1] = useState(currentUrl); 
    ...
    <>
      ...
      <Card style={{ width: "100%" }}>
        <Tabs activeKey={activeTabKey1} onChange={onTab1Change}>
          <TabPane
            tab={<Link to="/auth/signIn">sign in</Link>}
            key="/auth/signIn"
          >
          ...
          <TabPane
            tab={<Link to="/auth/signUp">sign up</Link>}
            key="/auth/signUp"
          >
